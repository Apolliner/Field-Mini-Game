...
class Path:
    ...
    def path_calculate(self, global_map, vertices_graph, vertices_dict, enemy_list):
        ...
        if not self.global_waypoints and not self.local_waypoints:
            ...
            if self.vertices == self.path_world_tile(global_map, self.memory.get_position()).vertices:
                self.local_waypoints, _ = self._path_world_tiles_a_star_algorithm(global_map,
                                                         self.world_position, self.memory.get_position(), self.vertices)
            # Если глобальные положения различаются
            else:
                #print(F"self.vertices - {self.vertices}, self.memory.get_vertices(global_map) - {self.memory.get_vertices(global_map)}")
                ...
                for connect in vertices.connections:
                    if connect.number == self.global_waypoints[0]:
                        if self.memory.get_vertices(global_map) == self.global_waypoints[0]:
                            _, approximate_position = self.path_world_position_recalculation(self.memory.get_position())
                        else:
