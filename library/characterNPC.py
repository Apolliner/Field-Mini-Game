...
class NPC(Character, Path, Bases):
    ...
    def npc_attack_calculations(self, action, global_map, vertices_graph, vertices_dict, enemy_list):
        """
            ...
            Необходима реализация следующей логики:
            Действие атаки делится на четыре типа: перемещение для атаки, действия для атаки, побег и атака.
                Перемещение для атаки Включает в себя выполнение определённой тактики. Такой тактикой можно считать
            обход противника по кругу при преградах на линии стрельбы. В таком случае выбирается направление для обхода,
            рассчитывается точка перемещения и до неё строится путь. На каждом шагу идёт проверка, а свободно ли на
            линии стрельбы. Если свободно, то производится выстрел или подготовка к выстрелу. Если цель сместилась на
            более 3 тайла, то путь пере рассчитывается. Важно понять как определять направление и не терять его, если
            надо обходить больше, чем полкруга.
                Действия для атаки включают в себя доставание оружия, прицеливание оружием и перезарядку оружия. Перед
            каждым выстрелом идёт оценка ситуации, надо стрелять прицельно, или быстро, от бедра, или вообще не
            стрелять. Прицеливание увеличивает точность, но во время прицеливания цель может скрыться с линии стрельбы.
            Перезарядка должна происходить, когда противника нет на линии стрельбы. Требуется находить точки, на которых
            противник не может произвести по персонажу выстрел, или выстрел будет затруднён.
                Атака может быть ближняя и дальняя. Атака включает в себя только выстрел/удар.
                Каждый шаг атаки оценивается необходимость побега или побега с отстреливанием. При побеге с
            отстреливанием персонаж раз в три-четыре шага оборачивается и пробует произвести прицельный или беглый
            выстрел. При просто побеге, персонаж убегает не выполняя никаких других действий.

                Порядок действий реализуется с помощью стека. В случае отсутствия заряженного оружия не получится
            выстрелить пока:
                Не будет найдено безопасное место для зарядки оружия
                Не будет заряжено оружие
                Не будет достигнута позиция для стрельбы
                Не будет прицеливания на противника
                И только теперь можно произвести выстрел
